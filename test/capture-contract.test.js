...
test("Manifest 保持独立运行所需的最小权限", () => {
  ...
  assert.match(background, /CONTENT_SCRIPT_REVISION = "article-more-menu-v20"/u);
...
test("Content Inbox 提供上下文感知收件箱、关注作者和素材库 Side Panel", () => {
  ...
  assert.match(content, /CONTENT_SCRIPT_REVISION = "article-more-menu-v20"/u);
...
test("Popup 与预览页保留清晰的成功、边界和来源反馈", () => {
  ...
  assert.match(contentScript, /const isSourcePage = isArticleSourcePage\(\);/u);
  ...
  assert.match(contentScript, /nativeFollowMenuIcon\(menu, isFollowing\)/u);
  assert.match(contentScript, /function nativeFollowMenuIcon/u);
  assert.match(contentScript, /labels = isFollowing \? \["Unfollow", "Following"\] : \["Follow"\]/u);
  ...
  assert.match(contentScript, /background: rgb\(232, 245, 253\) !important/u);
