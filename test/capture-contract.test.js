...
test("Popup 与预览页保留清晰的成功、边界和来源反馈", () => {
  ...
  assert.match(popupScript, /Markdown copied, but the native preview could not be opened/u);
  ...
  assert.match(contentScript, /right: 16px !important; bottom: 88px !important/u);
  ...
  assert.match(contentScript, /restoreNativePreview\(\);\n    createImportPanel\(\);/u);
