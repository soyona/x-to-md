...
test("Content Inbox 提供上下文感知收件箱、关注作者和素材库 Side Panel", () => {
  ...
  assert.match(script, /在 X 中原样预览/u);
  assert.match(script, /open-native-article-preview/u);
  assert.match(script, /previewCandidateOnX/u);
  assert.match(readFileSync(new URL("../background.js", import.meta.url), "utf8"), /chrome\.tabs\.create\(\{ url: sourceUrl \}/u);
  assert.match(readFileSync(new URL("../background.js", import.meta.url), "utf8"), /open-native-preview/u);
