...
- 入口语义以当前实现为准：扩展不监听 X 原生 Bookmark 的 hover/focus，也不注入 Bookmark 悬浮按钮；列表页 More 只处理候选集，`/status` 与 `/article` More 处理 `Extract and copy` 及素材库保存/移除。候选集使用托盘图标，素材库使用书签图标；底层 `x-to-md-content-inbox` 存储键保持兼容。
